...
# Fungi
## Download the NCBI taxonomy data required for the Kraken2 database construction in the "fungi_db" directory
kraken2-build --download-taxonomy --db fungi_db

## Build the Kraken2 database for fungi using the downloaded taxonomy in the "fungi_db" directory
kraken2-build --build --db fungi_db

## Add custom fungal sequences from "fungi.fasta" to the "fungi_db" database
kraken2-build --add-to-library fungi.fasta --db fungi_db
